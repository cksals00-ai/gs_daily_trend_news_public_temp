...
실행 시각: 2026-06-07 05:07 KST (자동 스케줄)
...
| 4. 전체 빌드 | ✅ | index.html, otb.html 재빌드 (Auto-Built 2026-06-07 05:06 KST). 내부 교차검증 통과 |
| 5. 숫자 검증 | ⚠️ | 단순 월합 비교는 측정범위 불일치로 무의미(정상) — 아래 참고 |
| 6. 커밋 | ✅ | `d487e1d` 로컬 커밋 (18 files changed, +352/−237) |
| 7. 푸시 | ❌ | **샌드박스 네트워크 차단**(SSH 443 Forbidden). origin/main 대비 **ahead 9 / behind 0**(클린 fast-forward) → 호스트 자동푸시 시 **rebase 불필요** |
...
- AI FCST RN=824,113 (CI 749,054~899,172) · RM FCST RN=158,719
- Daily Booking 보정 대상: 25.팔라티움(온북 DB 미포함 사업장)

## 빌드 내부 교차검증 (통과)
- 당일분석 NET=2,870 = 세그먼트합(OTA 1,387 + G-OTA 1,276 + IB 207) 일치 ✅
- OTB 합계 = 세그먼트합 일치 ✅
- 신호등 4개 권역 갱신(vivaldi 5/central 7/south 7/apac 3), 경쟁사 9개, 뉴스 234건 주입

## 숫자 검증 상세 (5단계 — 세부 로직 미확정)
온북 DB(stay-month net_rn, 온라인영업팀 채널 부분집합) vs Daily Booking Report actual_rns(전채널 PMS):

| 투숙월 | 온북DB | DailyBkRpt | 차이 | 차이% |
|---|---|---|---|---|
| 2026-06 | 63,581 | 157,878 | −94,297 | −59.7% ⚠️ |
| 2026-07 | 29,164 | 137,929 | −108,765 | −78.9% ⚠️ |
| 2026-08 | 2,779 | 90,909 | −88,130 | −96.9% ⚠️ |
| 2026-09 | 481 | 17,343 | −16,862 | −97.2% ⚠️ |
...
## 참고
- RM PDF는 신규 없음(최신 `Revenue Meeting_2026.06.01.pdf` 전일과 동일). Daily Booking PDF는 06.05자(신규 미수신, 전일 동일) 반영. 더미 데이터 미사용.
...
## 조치 필요
- **푸시**: 이번 커밋 `d487e1d`은 origin 대비 **ahead 9 / behind 0**(클린 상태)이므로 호스트(launchd / `_auto_commit_push.command`)가 단순 fast-forward로 push 가능. rebase 불필요. 샌드박스에서는 GitHub SSH(443) 차단으로 직접 push 불가.
