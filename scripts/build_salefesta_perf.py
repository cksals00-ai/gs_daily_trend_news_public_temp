...
def main() -> None:
    ...
    camp = cube.get("by_campaign", {}).get(CAMPAIGN_KEY)
    if not camp:
    ...
    logger.info(
        "✅ %s — RN %s · 객실매출 %.1f백만 · 스냅샷 %d일",
        OUTPUT_JSON.name, f"{camp['rn']:,}", camp["room_rev_m"], len(out["history"]),
    )
